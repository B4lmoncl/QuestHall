...
app/                  # Next.js app directory
  page.tsx            # Main dashboard component (~3640 lines)
  ...
  globals.css         # Tailwind + CSS utilities + animations (~2321 lines)
  layout.tsx          # Root layout wrapper
  DashboardContext.tsx # React context for shared state
components/           # React UI components (58 files, ~37k lines)
...
lib/                  # Backend business logic (9 files, ~5000 lines)
  state.js            # Central state, Maps, JSON persistence (~1427 lines)
  helpers.js          # Utility functions, paginate() (~2440 lines)
...
- **🔍 DIAGNOSTICS — check this FIRST when debugging runtime behaviour.** There is a private, rebuild-surviving event log at `GET /api/_diag` (`lib/diag.js`, `routes/diag.js`). It answers "what is the site showing right now, and why" without screenshots or guesswork. Auth via the `x-diag-key` header (or `?key=`); a wrong key returns 404 so the endpoint isn't discoverable. The key comes from `DIAG_KEY` in `.env`, else it is auto-generated into `data/diag-key.txt` (the `quest-data` docker volume, gitignored) — retrieve it with `docker compose exec api cat /app/data/diag-key.txt`.
  - `GET /api/_diag?player=X&limit=200&filter=board` → live snapshot + recent events. `GET /api/_diag/snapshot?player=X` → snapshot only.
  - The snapshot breaks open quests down by `withTemplateId` / `withoutTemplateId` / `poolBypassing` / `companion` / `npc` / creator, plus the player's pool sizes — exactly the numbers behind the recurring Quest Board floods.
  - `board.compose` is logged on every player board render (`shown`, `fromPool`, `bypass`, `poolSize`, `candidates`, `poolWasEmpty`) and flags an `ANOMALY` above 40 quests. A flood shows up instantly as a large `bypass`.
  - When adding a subsystem that can misbehave invisibly, log a `diagLog('<area>.<event>', {...counts})` there too. **Never log credentials, tokens, password hashes or e-mail addresses** — counts, ids and reasons only.
- **⚠ THE QUEST BOARD FLOOD TRAP — read before touching quest visibility.** The Quest Board must show ~11 open quests per player (the generated pool in `pp.activeQuestPool`). It has flooded with hundreds of quests **many times**, always through the same mistake: **using "a field is missing" as a reason to bypass the pool cap.** The filter in `getQuestsData()` (`routes/quests.js`) once read `openPlayer.filter(q => visibleIds.has(q.id) || !q.templateId)` — "quests without a templateId are hand-created, always show them". But `seedQuestCatalog()` (`lib/quest-catalog.js`) materialises **all ~594 catalog templates** as open quests, and companion quests are created without one either — so *hundreds* of quests matched `!q.templateId` and every single one bypassed the cap. Rules that MUST hold:
  1. **Never widen the pool bypass.** Only quests authored by a player or an NPC may bypass (`!q.templateId && q.createdBy !== 'system'`). Anything `createdBy: 'system'` belongs to the pool and is capped by it. Never re-add a bare `!q.templateId`.
  2. **Every auto-created quest MUST set `templateId`.** Missing it silently converts the quest into an uncapped "hand-created" one.
  3. **Never fall back to "show everything" when the pool is empty.** The empty-pool branch caps to ~11; `/api/dashboard` calls `ensurePlayerPool()` first so the pool is built before the board renders.
  4. **Verify empirically, never by reading code alone.** Boot the server, register a user, hit `/api/dashboard?player=X` several times and count `quests.open` — it must be ~11 and must not grow across calls. Three separate "fixes" to this bug were shipped without doing that and all three missed the real cause.
...
- `docs/archive/ITEM-SYSTEM-SPEC.md` — Gear/equipment design spec (affix pools, rarity, slots). **Implemented & archived.**
- `docs/archive/WOW-PROFESSION-REFACTOR.md` — **Definitive Profession Refactor Plan (implemented & archived).** WoW Classic style: 300 max skill, 8 professions (incl. Lederverarbeiter), ~866 recipes total, 91 materials, every recipe = unique named item template, intermediate materials (bars/bolts/cured leather), material specialization, BoE trading, WoW source distribution (40/30/20/10). Contains all design decisions + session-based implementation order. **READ THIS before any profession/crafting work.**
