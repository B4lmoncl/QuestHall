...
function forceSpawnMinimumNpc(now) {
  ...
  for (const qt of chain) {
    ...
    const quest = {
      id: `quest-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
      title: resolved.title, description: resolved.description || '', priority: resolved.priority || 'medium',
      type: resolved.type || 'personal', categories: [], product: null, humanInputRequired: false,
      ...
      coopClaimed: [], coopCompletions: [], skills: [], lore: resolved.lore || null,
      chapter: null, minLevel: 1, npcGiverId: giver.id, npcName: giver.name,
      npcRarity: giver.rarity, flavorText: resolved.flavorText || null,
      chainIndex: chain.indexOf(qt), chainTotal: chain.length,
      rarity: giver.rarity, rewards: resolved.rewards, npcRewards: resolved.rewards,
    };
    state.quests.push(quest);
    questIds.push(quest.id);
  }
  ...
  state.npcState.activeNpcs.push({ giverId: giver.id, arrivedAt, departureTime, expiresAt: departureTime, questChainIndex: chains.indexOf(chain) });
