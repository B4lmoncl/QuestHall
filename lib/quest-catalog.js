...
function seedQuestCatalog() {
  ...
  const seedQuests = templates.map((t, i) => ({
    ...
    // MUST be set: a quest without templateId is treated as "hand-created" and
    // bypasses the per-player quest pool cap. Omitting it here put all ~594
    // seeded catalog quests on every player's board at once.
    templateId: t.id,
  }));
