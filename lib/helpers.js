...
function calcDynamicForgeTemp(userId) {
  ...
  // Base decay: 2% per hour; Ausdauer stat: -0.5% decay rate per point
  const stats = _getRawStats(userId);
  const ausdauerReduction = (stats.ausdauer || 0) * 0.005;
  const decayRate = Math.max(0.2, 2 - ausdauerReduction); // floor at 0.2%/h
  const hoursElapsed = (Date.now() - new Date(u.forgeTempAt).getTime()) / 3600000;
