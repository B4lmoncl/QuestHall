...
function calcDynamicForgeTemp(userId) {
  ...
  // Collect completion timestamps from last 48h
  const completions = [];
  if (pp && pp.completedQuests) {
    ...
    for (const [qid, info] of Object.entries(pp.completedQuests)) {
      ...
      if (at >= cutoff) completions.push(at);
    }
  }

  if (completions.length === 0) return 0;

  // Each quest adds +10%, but decay -2% per hour since that quest
  let temp = 0;
  for (const at of completions) {
    const hoursAgo = (nowMs - at) / 3600000;
    const contribution = Math.max(0, 10 - hoursAgo * 2);
    temp += contribution;
  }
